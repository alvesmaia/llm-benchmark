...
def _parse_events(stdout: str) -> tuple[str | None, float | None]:
    """O `--format json` emite um evento JSON por linha. Extrai o sessionID e soma o custo
    dos eventos `step_finish` (cada passo reporta seu próprio `cost`)."""
    ...
    for line in stdout.splitlines():
        ...
        if ev.get("type") == "step_finish" and isinstance(part.get("cost"), (int, float)):
            ...
            saw_cost = True
    return session_id, (round(cost, 6) if saw_cost else None)
...
class OpenCodeCliAdapter(Adapter):
    ...
    def _run(self, phase: str, cmd: list[str], prompt: str, workdir: Path, env: dict,
        ...
        sid, cost = _parse_events(out)
        return PhaseResult(
            phase=phase, ok=(rc == 0), returncode=rc, stdout=out, stderr=err,
            duration_s=dur, session_id=sid or prev_sid, cost_usd=cost,
